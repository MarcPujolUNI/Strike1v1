...
def step_impl(context, map_name):
    context.browser.find_by_name("favourite_map").last.find_by_text(
        map_name
    ).click()
...
def step_impl(context, button_text):
    ...
    if "Save" in button_text:
        button = context.browser.find_by_css(
            "#save-button-container button"
        ).first
    else:
        button = context.browser.find_by_css(
            "#password-save-container button"
        ).first
    button.click()
...
def step_impl(context, expected_username):
    time.sleep(0.3)
    assert (
        context.browser.find_by_name("username").first.value
        == expected_username
    )
...
def step_impl(context, filename):
    asset_path = os.path.join(
        settings.BASE_DIR, "features", "steps", "assets", filename
    )
